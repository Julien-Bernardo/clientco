...
def compute_delivery_time(df_sample: pd.DataFrame) -> pd.DataFrame:
    ...
    df_sample["delivery_time"] = (
        df_sample["date_invoice"] - df_sample["date_order"]
...
def compute_time_last_order(df_no_outliers: pd.DataFrame) -> pd.DataFrame:
    df_churn = (
        df_no_outliers.groupby(["client_id", "date_order"]).agg(sum).reset_index()
    )
    df_churn["time_last_order"] = df_churn.groupby("client_id")[
        "date_order"
...
def compute_time_last_order_stats(df_churn: pd.DataFrame) -> pd.DataFrame:
    time_last_order_stats = df_churn.groupby(["client_id"]).agg(
        {"time_last_order": ["mean", "std"]}
    )
    df_churn = df_churn.merge(
        time_last_order_stats, left_on=["client_id"], right_index=True
    )
    return df_churn


def compute_churn(df_churn: pd.DataFrame) -> pd.DataFrame:
    df_churn["churn"] = df_churn["time_last_order"] - (
        df_churn[("time_last_order", "mean")] + 1 * df_churn[("time_last_order", "std")]
    )
...
def train_test_split(
    df: pd.DataFrame, split_date: str, DATE="date_order", TARGET="churn"
) -> pd.DataFrame:
    X_train = (
        df.query(f"{DATE} < @split_date").drop(TARGET, axis=1).reset_index(drop=True)
    )
